...
/*   Updated: 2020/08/02 17:05:53 by gbianco          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void			draw_ground_b(t_frame *t, t_vec2 *w, int *index)
{
	...
	cx = cx > 0 ? cx : 0;
	cy = cy > 0 ? cy : 0;
	t->sdl.color = floor[cy * 128 + cx] << 8 | 0xff;
...
void			draw_ground(t_frame *t)
{
	...
	{
		world_dist = (32. / (t->wall.end - (double)HHEIGHT));
		world_dist = t->cam.dist * world_dist / cos(t->cam.ang_c);
